...
- **Cross-ticket only** — if the same ticket affects both paths of a service, it does not count as a disruption (the provider is aware and handles it)
...
- Visual indicators (badges and "locked" tags) propagate into the Overlapping Ticket Pairs view so you can immediately see which overlaps have no flexibility
